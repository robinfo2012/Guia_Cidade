...
<section class="bg-gradient-to-r from-blue-600 to-blue-400 text-white py-20 text-center">
 ...
 <form class="max-w-2xl mx-auto flex bg-white rounded-lg overflow-hidden shadow-lg" method="get" action="">
  <input type="text" name="busca" placeholder="O que você procura?" class="flex-1 p-4 outline-none text-gray-700" value="<?= htmlspecialchars($_GET['busca'] ?? '') ?>">
  <button type="submit" class="bg-blue-600 px-6 text-white font-semibold hover:bg-blue-700 transition">Buscar</button>
</form>
...
<section id="categorias" class="max-w-7xl mx-auto px-6 py-16">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
    ...
    ?>
      <a href="?cat=<?php echo $c['id'];?>" class="bg-white shadow rounded-xl p-6 text-center hover:bg-blue-50 hover:shadow-md transition">
        <span class="text-lg font-semibold text-blue-600"><?php echo htmlspecialchars($c['name']);?></span>
      </a>
    <?php endwhile; ?>
  </div>
</section>
...
<section id="negocios" class="bg-gray-100 py-16">
  ...
  <img src="uploads/<?php echo htmlspecialchars($b['image']); ?>" 
